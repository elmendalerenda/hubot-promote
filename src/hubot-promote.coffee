  ...
  robot.respond /promote ([^:]+):?/i, (res) ->
    ...
    if typeof user is 'object'
      if user.name == robot.name
        ...
        res.send "I am really excited to announce that #{user.name} has been promoted to #{res.random(one)} #{res.random(two)} of #{res.random(three)} #{res.random(four)} #{res.random(five)}. #{user.name} has helped to #{res.random(adverbs)} #{res.random(verbs)} #{res.random(adjectives)} #{res.random(nouns)}. Please join me in congratulating #{user.name} on the news! :confetti_ball: :tada:"
